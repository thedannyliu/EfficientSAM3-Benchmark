...
SAM_BACKEND=null SAM_DEVICE=cpu SAM_MAX_FRAMES=5 \
  bash scripts/run_pace_thor_pipeline_smoke.sh videos/test1.mov
```

檢查輸出：

```bash
ls results/thor_pipeline_smoke/
ls overlays/thor_pipeline_smoke/
```
...
注意：目前 ROS wrapper 尚未把 `backbone_type`、`model_name`、`text_encoder_type` 暴露成 ROS parameters。若要在 Thor 上直接測多個 EfficientSAM3 variant，需要先補這些參數。
